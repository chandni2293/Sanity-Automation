...
class PaymentReportPage:
    def __init__(self, driver, base_url="https://afm2020.com/"):
        ...
        # ========= Replace these with actual locators =========
        ...
        self.CREATE_PAYMENT_DRIVER_TWO_XPATH = "your_xpath"
    ...
    def is_displayed(self, xpath):
        try:
            return self.wait.until(EC.visibility_of_element_located((By.XPATH, xpath))).is_displayed()
        except Exception:
            return False
    ...
    def enter_customer_details(self):
        try:
            ...
            customer_random = self.random_index(size_number)
            ...
            size_number = self.element_count(self.DROP_DOWN_OPTIONS_XPATH)
            sales_random = self.random_index(size_number)
    ...
    def enter_shipment_details(self):
        try:
            ...
            self.click(delivery_xpath)

            self.type_text(self.DELIVER_DATE_XPATH, drop_future_date)
            self.type_text(self.SHIPMENT_REFRENCE_NUMBER_XPATH, self.SHIPMENT_REFERENCE_NUMBER_COMMENT)
    ...
    def enter_revenue_details(self):
        try:
            ...
            self.type_text(self.REVENUE_FREIGHT_CHARGERATE_XPATH, self.REVENUE_FREIGHT_CHARGERATE_VALUE)
            self.select_by_visible_text(self.REVENUE_FUEL_RATE_METHOD_XPATH, self.REVENUE_RATE_METHOD_FLAT_TEXT)
    ...
    def fill_assert_details(self):
        try:
            ...
            truck_xpath = self.TRUCK_RANDOM_PICUP_LOCATION_START_PATH + self.truck_name + self.TRUCK_RANDOM_PICUP_END_PATH
            ...
            size_number = self.element_count(self.DROP_DOWN_OPTIONS_XPATH)
            self.trailer_name = self.random_index(size_number)
            trailer_xpath = self.DRIVER_TWO_START_PATH + self.trailer_name + self.DRIVER_TWO_END_PATH
            self.trailer_reference_name = self.get_text(trailer_xpath)
            self.click(self.DRIVER_NUMBER_START_PATH + self.trailer_name + self.DRIVER_NUMBER_END_PATH)
